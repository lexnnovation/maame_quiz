...
# Coolify's own healthcheck runs curl/wget via `docker exec` against the
# container (separate from the HEALTHCHECK directive below), so it needs to
# be present even though our own healthcheck uses Node's fetch instead.
